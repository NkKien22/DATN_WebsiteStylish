...
const UpdateProduct = () => {
  ...
  const porductUpdateDeleteOption = (el) => {
    ...
    newProductUpdate.ProductVariants?.map(e => e.VariantValues = e.VariantValues?.filter(obj => obj.OptionId != el.OptionId ));
    newProductUpdate.ProductOptions = newProductUpdate.ProductOptions?.filter(e => e.Id != el.Id);
    form.setFieldsValue({
  ...
  const uploadFile = (options) => {
    ...
    if (file) {
      ...
      dispatch(ProductServices.addImageProduct(formData))
        ...
        .then(async (res) => {
          if (res.status == 200) {
            file.fileUrl = res.data.files[0].fileUrl;
            onSuccess(res.data.files[0].fileUrl);
          } else {
            file.fileUrl = "";
            onError();
          }
